...
//        $pageSize = 2;
        ...
        $data = $this->db->select('*')->from('def_blog')->limit($offset, $pageSize)->query()->fetchAll();
        return $data;
    }
    ...
    public function getComments($bid){
       return $result = $this->db->querySql("select c.*,d.name from def_comments c left join def_user d on c.bid=" .$bid. " and c.uid=d.uid;");           
    }
    ...
    public function getReplies($bid){
        return $result = $this->db->querySql("select r.*,d.name fromName,u.name toName from def_replies r left join def_user d on r.bid=" .$bid. " and r.fromid=d.uid left join def_user u on r.toid=u.uid");
    }
}
